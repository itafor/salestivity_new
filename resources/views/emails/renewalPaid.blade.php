...
<table>
      <tr>
        <td colspan="2">
            ...
            <em>
             This is to confirm receipt of the sum of <strong>{!! $renewal && $renewal->renewal->currency ? $renewal->renewal->currency->symbol : '&#8358;' !!}{{number_format($renewal->amount_paid,2)}}</strong> for the <strong>{{$renewal->product->name}}</strong> for <strong>{{ $renewal->customer->name }}</strong>
             <br/>
      ...
      <table class="table table-bordered" id="rental_table">
           @if(isset($renewal))
                    ...
                    <tr>
                     ...
                     <td>{!! $renewal && $renewal->renewal->currency ? $renewal->renewal->currency->symbol : '&#8358;' !!}{{ number_format($renewal->renewal->billingAmount,2) }}
                     </td>
                   </tr>

                    <tr>
                     <td style="width: 120px;"><b>{{ __('Amount Paid') }}</b></td>
                     <td>{!! $renewal && $renewal->renewal->currency ? $renewal->renewal->currency->symbol : '&#8358;' !!}{{ number_format($renewal->renewal->amount_paid,2) }}
                     </td>
                   </tr>
                      <tr>
                ...
                <td>{{ date("jS F, Y", strtotime($renewal->payment_date)) }}</td>           
                   ...
                   <tr>
                     ...
                     <td>{!! $renewal && $renewal->renewal->currency ? $renewal->renewal->currency->symbol : '&#8358;' !!}{{ number_format($renewal->billingbalance,2) }}
                     </td>
                   </tr>
