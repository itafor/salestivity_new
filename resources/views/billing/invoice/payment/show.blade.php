        ...
        <tbody>
            ...
            @foreach($invoice->invoicePayment as $payment)
                <tr>
                    ...
                    <td>{!! $invoice->prod && $invoice->prod->currency ? $invoice->prod->currency->symbol : '&#8358;' !!}{{ number_format($payment->billingAmount,2)}}</td>
                    <td>{!! $invoice->prod && $invoice->prod->currency ? $invoice->prod->currency->symbol : '&#8358;' !!}{{ number_format($payment->amount_paid,2)}}</td>
                    <td>{!! $invoice->prod && $invoice->prod->currency ? $invoice->prod->currency->symbol : '&#8358;' !!}{{ number_format($payment->billingbalance,2)}}</td>
